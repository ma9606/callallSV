...
do
   ...
   sort -k6,6nr .tmp.mergePE-iBP_tested.list > mergePE-iBP_tested.list  &&  rm .tmp.mergePE-iBP_tested.list
  
   # awk -f $RSC/prepFmt_SVlist.awk  mergePE-iBP_tested.list  > merged_SV.trimFmt.list
   # mv  mergePE-iBP_tested.list  merged_SV.list
...
do
   ...
   cd ${HDR}/Merge/${SID}
   cp -p  $CUR/${SID}/mergePE-iBP_tested.list.PEsupR ./merged_SV.list  &&  rm mergePE-iBP_tested.list
   awk -f $RSC/trimFmt_rearrangement10.awk  merged_SV.list > merged_SV.trimFmt.list
