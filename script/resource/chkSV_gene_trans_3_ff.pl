...
#		   ($gene[$i]->{cds_exon_sta}[0] <= $end1 && $end1 < $gene[$i]->{cds_exon_end}[0])){
		...
		elsif(($gene[$i]->{cds_exon_sta}[$cds_exon_num - 1] <= $end1 && $end1 < $gene[$i]->{cds_exon_end}[$cds_exon_num - 1]) ||
...
#			   ($gene[$i]->{cds_exon_sta}[$j+1] <= $end1 && $end1 < $gene[$i]->{cds_exon_end}[$j+1])){
	...
	if($chr2 eq $gene[$i]->{chr}){
	    if($gene[$i]->{gene_sta} < $sta2 && $sta2 <= $gene[$i]->{gene_end}){
